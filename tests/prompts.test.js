...
describe("worker prompts", () => {
  ...
  it("國語 prompt 套用 qualityMeta / distractorDesign 壓縮契約", () => {
    ...
    expect(chinesePrompt).toContain("同一題三個錯誤選項的 misconceptionTag 不得三個完全相同");
    expect(chinesePrompt).toContain("不要預設使用 structure_confusion");
    expect(chinesePrompt).toContain("每個錯誤選項只寫一個核心迷思");
    expect(chinesePrompt).toContain("不要在每個錯誤選項重複題幹、正答解析、完整文本內容、選項全文或完整教學說明");
    expect(chinesePrompt).not.toContain("數學 qualityMeta / distractorDesign 壓縮規則");
