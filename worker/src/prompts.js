...
export function buildGenerateItemsPrompt({ project = {}, materialText = "", objectives = [], intents = [] }) {
  ...
  return [
    ...
    "- 若 questionType 為「圖表判讀題」，請先在 question 內以「文字表格」呈現一份資料（例如實驗記錄表、觀察統整表、數據或統計表；用 Markdown 表格或對齊的文字表格），再設計 2 至 3 個子題測「圖表判讀」能力（讀值、比較大小、找出規律、依資料推論）；question 需完整包含該表格與 (1)(2)(3) 子題，answer 逐一列出各子題答案，該題配分為整題總分。表格數據需合理、與教材概念相符。",
    "",
    "# 題目品質要求",
