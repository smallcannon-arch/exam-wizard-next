...
export function buildGenerateItemsPrompt({ project = {}, materialText = "", objectives = [], intents = [], checkedChineseSubcategories = [], fewShotExamples = FEWSHOT_EXAMPLES }) {
  ...
  const promptParts = [
    ...
    "- 【stimulus contract】除非該 item 的 stimulus 欄位提供完整閱讀文本，question 不得使用「根據這段文字」「根據本文」「根據上文／下文」「文中」等依賴外部文本的說法；一般題目必須自成一題。",
    "- 若沒有 stimulus，question 必須直接寫出作答所需資訊，不得要求學生參照未提供的本文、上文或這段文字；違反此規則會被 Worker 以 AI_STIMULUS_MISSING 拒絕。",
    "",
