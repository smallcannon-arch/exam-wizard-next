...
export function buildGenerateItemsPrompt({ project = {}, materialText = "", objectives = [], intents = [] }) {
  ...
  const promptParts = [
    ...
    "",
    "# 學力檢測題（情境素養題組）之特別規定",
    "若題位的 questionType 為「學力檢測題」，你必須將其拆解成 2 到 3 個子題。每一題（子題）在輸出 JSON 的 items 陣列中作為一個獨立的物件。請嚴格遵守以下規定：",
    ...
    "  3. 各子題必須是獨立的物件，具有自己的 `itemId`, `questionType` (為 `\"學力檢測題\"`), `score` 等。",
    ...
    "  7. 每個子題有自己獨立的 `options`（4個選項）、`answer`（單一正確答案，A, B, C 或 D）與 `explanation`。",
    ""
  ];
