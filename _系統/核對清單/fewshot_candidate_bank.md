...
狀態：候選庫；第一輪已准入 4 題進入正式提示詞，其餘題目仍需經 Smallcannon 准入審核後，才可改為 `prompt_ready`。

## 使用原則

- 本庫用於建立 few-shot 題感種子，重點是「標註品質判準」，不是累積大量題目。
- 題目以公開學力檢測 / TASA 題感與試題分析為參考來源，但不直接照抄原題。
- 未准入題目仍維持 `candidate`，`promptUseStatus` 維持 `not_ready_until_teacher_review`。
...
- 准入日期：2026-06-20
- 審核者：Smallcannon
- 准入用途：第一輪 A/B 測試用 few-shot。
- 准入題目：G4_CH_READ_001、G4_CH_SENTENCE_003、G4_MA_CLOCK_002、G4_MA_EQUATION_004。
- 條件註記：G4_MA_CLOCK_002 限於四年級 S-4-2 旋轉角與鐘面模型，不處理超過 360 度或時針分針複合夾角公式。
